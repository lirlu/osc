...
var _Data = {'page':0, 'limit':15, 'type':'3', 'key':app.store('key')};
...
function next (cb) {
	...
	$.ajax({
		'dataType' : 'json',
		'type'     : 'post',
		'url'      : app.url('mobile/order/d_commont_list'),
		'data'     : _Data
	})
	.fail(function (res) {
		console.log('取得未评论数据失败：' + JSON.stringify(res));
		app.error('取得未评论数据失败');
		plus.nativeUI.closeWaiting();
	})
	.done(function (res) {
		console.log('未评论数据：' + JSON.stringify(res));
		plus.nativeUI.closeWaiting();
...
function funcPulldownRefresh () {
	...
	next(function (res) {
		plus.nativeUI.closeWaiting();
		$('#pnl-order').html(template('tpl-todo', res));
...
function funcPullupRefresh () {
	next(function (res) {
		...
		$('#pnl-order').append(template('tpl-todo', res));
...
// 评论订单
$('body').delegate('.btn-odr-comment', 'tap', function () {
	var dom  = this, odr = $(dom).closest('.order');
