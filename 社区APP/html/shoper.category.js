...
mui.plusReady(function () {
	app.log('获取水站分类：' + app.url('mobile/cateseller/union_seller_cate'));
	...
	.fail(function (res) {
		app.log('取得联盟商家分类失败：' + JSON.stringify(res));
		app.error('取得联盟商家分类失败');
		plus.nativeUI.closeWaiting();
	})
	.done(function (res) {
		app.log('联盟商家分类：' + JSON.stringify(res));
		plus.nativeUI.closeWaiting();
...
function category (id) {
	// 切换TAB页回本社区
	var js = '$(\'[data-page="shoper.scroll.html"]\').addClass("activet").siblings().removeClass("activet");'
	plus.webview.getWebviewById('shoper.html').evalJS(js);
	// 重新加载数据
	var view = plus.webview.getWebviewById('shoper.scroll.html');
